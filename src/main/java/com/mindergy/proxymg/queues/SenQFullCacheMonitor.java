...
/**
 * This class monitors the senq table and the cached data.
 * @author TIMwe
 *
 * TODO To change the template for this generated type comment go to
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class SenQFullCacheMonitor extends Thread
{
    ...
    /**
     * Number of msgs batch processors will handle
     */
  ...
  /**
   * This method etablished the father for this class
 * @param main
 */
public SenQFullCacheMonitor(ProxyMain main)
  {
    this.main=main;
  }
    ...
    /**
     * This method is the thread starting point
     * @author Paulo Salgado
     * @version 1.0.2
     */
    public void run()
    {
      ...
      try
      {
          ...
          Log.info("SENQ CACHED MON for xconn " + this.xConnKey + " started");
          ...
          Date start = new Date();    
              if(alivec%1000==0)
                Log.info("SENQMONITOR "+this.xConnKey + " I AM ALIVE!!!");
              else if(alivec%15==0)
              ...
              Date end=new Date();
              if(alivec%100==0)
                  Log.info("SENQMON CACHED " + this.xConnKey + " getQ:" + (end.getTime()-start.getTime())+ " bef: " + bef + " ids:" + this.runningIds.size());
          ...
          Log.info("SENQ CACHED MON for xconn " + this.xConnKey + " stopped");
      }
      catch(Exception e)
      {
        Log.error(e);
    ...
    /**
       ...
       * to get the requests to process updates each row 
       * @author Paulo Salgado
       * @version 1.0.0
       */
    private void getQueueEntries()
    {
      ...
      try
      {
          ...
          if(targetLines>0)
          {
		      ...
		      s = db.prepareStatement(ProxyXconn.getXconnReplacedSql(SQLData.Senq.SELN_MON,this.xConnKey));
		      su = db.prepareStatement(ProxyXconn.getXconnReplacedSql(SQLData.Senq.UPD_MON_STATUS,this.xConnKey));
		      s.setInt(1,0);
		      s.setInt(2,targetLines);
	        ...
	        while (rs.next() && count<targetLines)
	        {
	            ...
	            sms.setPurposeId(rs.getInt(21));
	            sms.setSender(rs.getString(22));
...
//        Always make sure result sets and statements are closed,
    ...
    private void launchProcessorThreads() 
    {
        try
        {
            ...
            {
                Log.debug("SENQ Cache Monitor prepare to launch thread ");
                if (ending)
                {
                ...
                // get the class
                SenQFullCacheProcessor processor = new SenQFullCacheProcessor(this.main, this);
                // lauch
                processor.start();
