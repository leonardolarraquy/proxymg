...
/**
 * This class monitors the chargeq table and the cached data.
 * @author TIMwe
 *
 * TODO To change the template for this generated type comment go to
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class ChargeQFullCacheMonitor extends Thread
{
    ...
    /**
     * Number of lines to retrieve in eacg iteration
     */
  ...
  /** 
   * This method establishes the relationship with the main
   * class.
 * @param main
 */
public ChargeQFullCacheMonitor(ProxyMain main)
  {
    this.main=main;
  }
    ...
    public void run()
    {
      ...
      try
      {
          ...
          Log.info("CHARGEQ CACHED MON for xconn " + this.xConnKey + " started");
          ...
          Date start = new Date();    
              if(alivec%1000==0)
                Log.info("CHARGEQMONITOR "+this.xConnKey + " I AM ALIVE!!!");
              else if(alivec%15==0)
              ...
              Date end=new Date();
              if(alivec%100==0)
                  Log.info("CHARGEQMON CACHED " + this.xConnKey + " getQ:" + (end.getTime()-start.getTime())+ " bef: " + bef + " ids:" + this.runningIds.size());
          ...
          Log.info("CHARGEQ CACHED MON for xconn " + this.xConnKey + " stopped");
      }
      catch(Exception e)
      {
        Log.error(e);
    ...
    private void getQueueEntries()
    {
      ...
      try
      {
          ...
          if(targetLines>0)
          {
		      ...
		      s = db.prepareStatement(ProxyXconn.getXconnReplacedSql(SQLData.Chargeq.SELN_MON,this.xConnKey));
		      su = db.prepareStatement(ProxyXconn.getXconnReplacedSql(SQLData.Chargeq.UPD_MON_STATUS,this.xConnKey));
		      s.setInt(1,0);
...
//        Always make sure result sets and statements are closed,
    ...
    private void launchProcessorThreads() 
    {
        try
        {
            ...
            {
                Log.debug("CHARGEQ Cache Monitor prepare to launch thread ");
                if (ending)
                {
                ...
                // get the class
                ChargeQFullCacheProcessor processor = new ChargeQFullCacheProcessor(this.main, this);
                // lauch
                processor.start();
